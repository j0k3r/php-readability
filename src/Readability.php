...
class Readability implements LoggerAwareInterface
{
                ...
                if (!preg_match($this->regexps['divToPElements'], $nodeContent)) {
                    ...
                    foreach ($childs as $childNode) {
                        ...
                        if ($this->isPhrasingContent($childNode)) {
                            ...
                            } else if ('' !== $this->getInnerText($childNode, true, true)) {
        ...
        if (null === $topCandidate || 0 === strcasecmp($topCandidate->tagName, 'body')) {
            ...
            foreach ($topCandidates as $candidate) {
                if ((int) $candidate->getAttribute('readability') / (int) $topCandidate->getAttribute('readability') >= 0.75) {
                    ...
                    $this->logger->debug('Adding ' . \sizeof($ancestors) . ' alternative ancestors for ' . $candidate->getNodePath());
                    array_push($alternativeCandidateAncestors, $ancestors);
                }
            }
            if (\sizeof($alternativeCandidateAncestors) >= 3) {
                $parentOfTopCandidate = $topCandidate->parentNode;
                while ($parentOfTopCandidate->nodeName !== "body") {
                    $listsContainingThisAncestor = 0;
                    for ($ancestorIndex = 0; $ancestorIndex < \sizeof($alternativeCandidateAncestors) && $listsContainingThisAncestor < 3; $ancestorIndex++) {
            ...
            $parentOfTopCandidate = $topCandidate->parentNode;
            while ($parentOfTopCandidate->nodeName !== 'body' && $parentOfTopCandidate->childNodes->length == 1) {
                $topCandidate = $parentOfTopCandidate;
                $parentOfTopCandidate = $topCandidate->parentNode;
            }
            if (!$topCandidate->hasAttribute('readability')) {
        ...
        for ($s = 0, $sl = $siblingNodes->length; $s < $sl; ++$s) {
            ...
            if ($siblingNode->isSameNode($topCandidate)) {
                ...
                if (\XML_ELEMENT_NODE === $siblingNode->nodeType && $siblingNode->hasAttribute('readability') && (((int) $siblingNode->getAttribute('readability')) + $contentBonus) >= $siblingScoreThreshold) {
                    $append = true;
                } else if (0 === strcasecmp($siblingNodeName, 'p')) {
                    ...
                    if (($nodeLength > self::MIN_NODE_LENGTH && $linkDensity < self::MAX_LINK_DENSITY)
                        || ($nodeLength < self::MIN_NODE_LENGTH && $nodeLength === 0 && 0 === $linkDensity && preg_match('/\.( |$)/', $nodeContent))) {
                        $append = true;
                    }
                }
            }
    ...
    private function getAncestors(\DOMElement $node, int $maxDepth = 0)
    {
        $ancestors = [];
        $i = 0;
    ...
    private function hasSingleTagInsideElement($node, $tag) {
        if ($node->childNodes->length !== 1 || $node->childNodes->item(0)->nodeName !== $tag) {
            return false;
        }
